...
async def search_in_index(
    ...
    if index_dir not in INDEX_CACHE:
        ...
        # ── SQLite docstore (fast) vs JSON docstore (legacy) ──
        sqlite_path = os.path.join(index_dir, SQLITE_DOCSTORE_FNAME)
        if os.path.exists(sqlite_path):
            logger.info(f"⚡ Loading SQLite docstore: {sqlite_path}")
            docstore = SqliteDocumentStore.from_persist_path(sqlite_path)
            storage_context = StorageContext.from_defaults(
                vector_store=vector_store,
                docstore=docstore,
                persist_dir=index_dir  # still needed for index_store.json
            )
        else:
            logger.info(f"📂 Loading JSON docstore (legacy): {index_dir}")
            storage_context = StorageContext.from_defaults(
                vector_store=vector_store,
